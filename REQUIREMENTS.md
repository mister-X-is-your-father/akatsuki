...
| **Database & Interface** | **Obsidian** (Local Markdown Files) | データベースとして機能し、ユーザーインターフェースでもある。**相互リンク機能（`[[リンク]]`）を活用**してナレッジ、ルール、エージェント間の関係を可視化・探索可能にする |
...
2. **Knowledge Sharing（ナレッジ共有）**
   ...
   - **Obsidian相互リンクの自動生成:**
     - 関連ナレッジへのリンク: `[[95_Library/00_Global/related_knowledge]]`
     - 関連ルールへのリンク: `[[90_Rules/00_Global/related_rule]]`
     - 作成エージェントへのリンク: `[[96_Agents/[agent_id]/]]`
     - 関連タスクへのリンク: `[[00_Strategy_Input/task_id]]`
     - 関連成果物へのリンク: `[[20_Approved_Outputs/output_id]]`
...
- **ナレッジ構造の改善:**
  - 未活用原因に基づいて、ナレッジ構造を自動的に改善：
    ...
    - **関連ナレッジのリンク:** 関連するナレッジ間のObsidian相互リンク（`[[リンク]]`）を追加
...
- **処理:**
  ...
  - **Obsidian相互リンクの自動生成:**
    - 関連ルールへのリンク: `[[90_Rules/00_Global/related_rule]]`
    - 関連ナレッジへのリンク: `[[95_Library/00_Global/related_knowledge]]`
    - 更新者エージェントへのリンク: `[[96_Agents/[agent_id]/]]`
    - 関連レビューログへのリンク: `[[96_Agents/[agent_id]/work/review_logs/review_id]]`
    - 関連タスクへのリンク: `[[00_Strategy_Input/task_id]]`
...
### 5.5 Obsidian相互リンク機能の活用 (Obsidian Bidirectional Links)

#### 5.5.1 相互リンクの基本原則

- **Obsidianの相互リンク機能を前提とした設計:**
  - すべてのMarkdownファイルはObsidianの相互リンク機能（`[[リンク]]`）を活用
  - リンクは双方向に機能し、関連ファイル間の関係を可視化
  - Obsidianのグラフビューでナレッジ、ルール、エージェント間の関係を可視化可能
...
- **team_map.md のリンク地図:**
  - 各エージェントへのリンク: `[[96_Agents/[agent_id]/]]`
  - 組織階層の可視化（supervisor_id関係）
  - 部門別のグループ化
- **エージェント作業ファイルのリンク:**
  - ドラフトから関連ナレッジ、ルールへのリンク
  - レビューログから関連ルール、ナレッジへのリンク
  - パフォーマンス記録から関連タスクへのリンク

#### 5.5.5 タスクと成果物の相互リンク

- **タスクファイル（00_Strategy_Input/）のリンク:**
  - 担当エージェントへのリンク: `[[96_Agents/[agent_id]/]]`
  - 関連ナレッジへのリンク: `[[95_Library/00_Global/knowledge]]`
  - 関連ルールへのリンク: `[[90_Rules/00_Global/rule]]`
  - 成果物へのリンク: `[[20_Approved_Outputs/output_id]]`
- **成果物ファイル（20_Approved_Outputs/）のリンク:**
  - 作成エージェントへのリンク: `[[96_Agents/[agent_id]/]]`
  - 関連タスクへのリンク: `[[00_Strategy_Input/task_id]]`
  - 参照したナレッジへのリンク: `[[95_Library/00_Global/knowledge]]`
  - 参照したルールへのリンク: `[[90_Rules/00_Global/rule]]`

#### 5.5.6 レビューログの相互リンク

- **レビューログファイルのリンク:**
  - レビュー対象エージェントへのリンク: `[[96_Agents/[agent_id]/]]`
  - レビュー対象ドラフトへのリンク: `[[96_Agents/[agent_id]/work/drafts/draft_id]]`
  - 関連ルールへのリンク（指摘内容に関連するルール）: `[[90_Rules/00_Global/rule]]`
  - 関連ナレッジへのリンク（参照すべきだったナレッジ）: `[[95_Library/00_Global/knowledge]]`
  - 関連タスクへのリンク: `[[00_Strategy_Input/task_id]]`

#### 5.5.7 相互リンクの自動生成とメンテナンス

- **自動リンク生成:**
  - ファイル作成時に自動的に関連リンクを生成
  - ファイル更新時にリンクを自動更新
  - リンクの整合性チェック（存在しないファイルへのリンクの検出）
- **リンクのメンテナンス:**
  - ファイル移動時のリンク自動更新
  - ファイル削除時のリンク自動削除
  - 孤立リンク（リンク先が存在しない）の検出と警告

#### 5.5.8 実装要件

- Obsidianの相互リンク機能を前提とした実装
- リンクの自動生成機能を実装
- リンクの整合性チェック機能を実装
- リンクのメンテナンス機能を実装

### 5.6 パフォーマンス最適化 (Performance Optimization)
...
### 6.20 Obsidian相互リンク機能
- [ ] ナレッジファイル作成時の自動リンク生成機能
- [ ] ルールファイル作成・更新時の自動リンク生成機能
- [ ] エージェント作業ファイルの自動リンク生成機能
- [ ] タスク・成果物ファイルの自動リンク生成機能
- [ ] レビューログの自動リンク生成機能
- [ ] リンクの整合性チェック機能（存在しないファイルへのリンク検出）
- [ ] リンクの自動更新機能（ファイル移動時）
- [ ] 孤立リンクの検出と警告機能
- [ ] team_map.md のリンク地図自動生成機能
...
| **Obsidian Bidirectional Links** | Obsidianの相互リンク機能（`[[リンク]]`）。ナレッジ、ルール、エージェント間の関係を可視化・探索可能にする |
| **Graph View** | Obsidianのグラフビュー機能。システム全体の関係性を可視化 |
...
### v1.5 (2025-12-26)
- Obsidian相互リンク機能の活用
  - すべてのMarkdownファイルでObsidianの相互リンク機能（`[[リンク]]`）を活用
  - ナレッジ、ルール、エージェント間の関係を可視化・探索可能にする
  - リンクの自動生成とメンテナンス機能を追加
  - Obsidianのグラフビューでシステム全体の関係性を可視化
  - ドラフト、ナレッジ、ルール、タスク、成果物、レビューログの相互リンク自動生成
